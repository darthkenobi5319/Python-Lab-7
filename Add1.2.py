# -*- coding: utf-8 -*-
"""
Created on Tue Oct  3 14:41:10 2017
...
'''    
#fight club version2
newList=dragonList.copy()
i=1
while True:
    if i >= (len(newList)-1):
        break
    if newList[i-1]['breed']==newList[i]['breed'] or newList[i]['breed']==newList[i+1]['breed']:
        i+=1
        continue
    if newList[i-1]['size']<=newList[i]['size'] and newList[i-1]['size']<=2*newList[i]['size']:
        newList.pop(i-1)
        continue
    elif newList[i-1]['size']<=newList[i]['size']:
        i+=1
        continue
    elif newList[i-1]['size']>newList[i]['size']:
        if 2*newList[i-1]['size']>=newList[i]['size']:
            newList.pop(i)
            continue
        elif newList[i]['size']<newList[i+1]['size']:
            newList.pop(i)
            continue
        else:
            i+=1
            continue
if newList[-2]['size']*2 >= newList[-1]['size']:
    newList.pop(-1)
print(newList)
print('There are',len(newList),'dragons left.')
